...
describe("Testing LMCV", function () {
    ...
    describe("Repay() testing", function () {
        ...
        //     console.log(await lmcv.getWeightedRate(addr1.address));
    ...
    // describe("getWeightedRate() testing", function () {
    //     it("When dust level is set to be above loan amount, this leads to unhealthy account", async function () {
    //         await lmcv.collatRate(mockTokenBytes, fray("1.05"));
    //         await lmcv.collatRate(mockToken2Bytes, fray("1.10"));
    //         await lmcv.collatRate(mockToken3Bytes, fray("1.30"));
    //         await userLMCV.loan(collateralBytesList, [fwad("50"), fwad("100"), fwad("200")], fwad("1000"), addr1.address);
    ...
    //     });
    // });
    describe("rates functionality testing", function () {
    //--------------------------------------------------
        // it("Should fail when not repaying amount of dPrime needed to unlock collateral", async function () {
        //     await lmcv.collatRate(mockTokenBytes, fray("1.05"));
        //     await lmcv.collatRate(mockToken2Bytes, fray("1.10"));
        ...
        //     expect(await lmcv.dPrime(addr1.address)).to.equal(frad("893.55"));
        //     expect(await lmcv.normalDebt(addr1.address)).to.equal(fwad("777"));
        ...
        //     await userLMCV.loan(collateralBytesList, [fwad("50"), fwad("100"), fwad("200")], fwad("2608.6956"), addr1.address);
        ...
        //     let userDPrimeJoin = dPrimeJoin.connect(addr1);
        //     await userLMCV.proxyApprove(userDPrimeJoin.address);
        //     await userDPrimeJoin.exit(addr1.address, fwad("1130.43"));

        //     await expect(
        //         userLMCV.repay([mockToken3Bytes, mockToken2Bytes], [fwad("200"), fwad("100")], fwad("1625.71299"), addr1.address)
        //     ).to.be.revertedWith("LMCV/More dPrime left than allowed");
        // });
        ...
        it("Should fail when not repaying amount of dPrime needed to unlock collateral", async function () {
            ...
            await userLMCV.loan(collateralBytesList, [fwad("50"), fwad("100"), fwad("200")], fwad("2630"), addr1.address);

            console.log("Weighted Rate: " + (await lmcv.getWeightedRate(addr1.address)).toString());
            ...
            await userLMCV.repay([mockToken3Bytes, mockToken2Bytes], [fwad("200"), fwad("100")], fwad("1691"), addr1.address);
            await userLMCV.loan([mockToken2Bytes, mockToken3Bytes], [fwad("100"), fwad("200")], fwad("1691"), addr1.address);

            console.log("Weighted Rate: " + (await lmcv.getWeightedRate(addr1.address)).toString());
            console.log(await lmcv.normalDebt(addr1.address));
            console.log(prad(await lmcv.dPrime(addr1.address)));

            // for (let i = 0; i < 1678; i++) {
            //     await expect(userLMCV.repay([mockToken3Bytes, mockToken2Bytes], [fwad("200"), fwad("100")], fwad(i+""), addr1.address)).to.be.reverted;
            // }

            // for (let i = 1679; i < 2631; i++) {
            //     console.log(i);
            //     console.log((2630 - i)+"");
                
            //     await userLMCV.repay([mockToken3Bytes, mockToken2Bytes], [fwad("200"), fwad("100")], fwad(i+""), addr1.address);
                
            //     let dPR = await lmcv.dPrime(addr1.address);
            //     let ND = await lmcv.normalDebt(addr1.address);
            //     let WR = await lmcv.getWeightedRate(addr1.address);
            //     expect(dPR.lte(ND.mul(WR))).to.be.true;
            //     await userLMCV.loan([mockToken2Bytes, mockToken3Bytes], [fwad("100"), fwad("200")], fwad(i+""), addr1.address);
            // }

            //THIS SHOULD DEFINITELY NOT BE GREATER THAN THE AMOUNT YOU HAVE TO REPAY BUT IT IS

...
async function setupCollateral(){
    ...
    let ret = await collateralJoinFactory.deploy(lmcv.address, lmcvProxy.address, CHAINLINKBYTES, "0x01BE23585060835E02B77ef475b0Cc51aA1e0709");
    console.log(ret);
...
async function addPerms(){
    ...
    await lmcv.administrate("0xbb2EbebC17CAf0cD430965912632615aF9611273", 1); 

    console.log("Setting collateral types");
    await lmcv.editAcceptedCollateralType(CHAINLINKBYTES, fwad("1000000"), fwad("1"), fray(".7"), false); 

    console.log("Setting spot prices");
    await lmcv.updateSpotPrice(USDCBytes, fray("7.31"));
...
async function lmcvProxySetup(){
    ...
    await lmcvProxy.editCollateral(CHAINLINKBYTES, "0xbb2EbebC17CAf0cD430965912632615aF9611273", "0x01BE23585060835E02B77ef475b0Cc51aA1e0709", MAX_INT);
